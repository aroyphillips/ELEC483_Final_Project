function result = demo_pgplvm_on_gaus(xxtrue,yytrue,fftrue,lflag)
%
...
setopt.niter = 5000; % number of iterations
...
%     end
    ...
    subplot(313)
    plot([yymat(:,yi),exp(ffmat(:,yi))])
    title('Firing Rates')
    title(sprintf("Biggest firing rate:, neuron %g",yi));
    ylabel('Firing Rate')
    xlabel("time")
    legend('true ff','P-GPLVM ff'), axis tight % drawnow
    gg = size(yymat,1);
    tmp = yymat(1:gg,:)./repmat(max(yymat(1:gg,:))+1e-6,gg,1);
    subplot(311), imagesc(tmp'), colorbar
    title("True spike counts")
    ylabel("neuron")
    xlabel("time")
    ff1 = exp(ffmat);
    tmp = ff1(1:gg,:)./repmat(max(ff1(1:gg,:))+1e-6,gg,1);
    subplot(312), imagesc(tmp'), colorbar
    title("Estimated spike counts")
    ylabel("neuron")
    xlabel("time")
    end
%     drawnow
    ...
    for dd = 1:nf
        ...
        title(sprintf('latent variable dimension: %g',dd))
